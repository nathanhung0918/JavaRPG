...
public class GameOverState extends GameState {
	...
	public void draw(Graphics2D g) {
		...
		Content.drawString(g, "finish time", 35, 36);
		
		int minutes = (int) (ticks / 1800);
		int seconds = (int) ((ticks / 30) % 60);
		if(minutes < 10) {
			if(seconds < 10) Content.drawString(g, "0" + minutes + ":0" + seconds, 54, 48);
			else Content.drawString(g, "0" + minutes + ":" + seconds, 57, 48);
		}
		else {
			if(seconds < 10) Content.drawString(g, minutes + ":0" + seconds, 54, 48);
			else Content.drawString(g, minutes + ":" + seconds, 57, 48);
		}
		
		Content.drawString(g, "rank", 58, 66);
		if(rank == 1) Content.drawString(g, "speed demon", 35, 78);
		else if(rank == 2) Content.drawString(g, "adventurer", 35, 78);
		else if(rank == 3) Content.drawString(g, "beginner", 45, 78);
		else if(rank == 4) Content.drawString(g, "bumbling idiot", 23, 78);
